...
5. 结论
    ...
    | 缺点 | 说明 | 日志分析场景的影响 |
    |------|------|----------------|
    | **不支持复杂脚本结构** | ❌ 不支持 `if/else`、`for`、变量赋值、多语句块。 | 无法写“如果失败3次且IP在黑名单，则告警”这类逻辑。 |
    | **无热更新机制** | ❌ 表达式通常在启动时编译，动态加载需手动管理缓存/解析器。 | 无法让安全人员动态修改规则并立即生效。 |
    | **性能中等** | ⚠️ 每次执行需解析或缓存，性能不如 QLExpress/Aviator。 | 高吞吐日志流下可能成为瓶颈。 |
    | **无沙箱/安全控制** | ❌ 可调用任意方法（如 `T(java.lang.Runtime).getRuntime().exec(...)`），有安全风险。 | 不可控脚本可能被注入执行任意代码。 |
    | **调试困难** | ❌ 错误信息不友好，无行号定位。 | 安全人员写错脚本难排查。 |
    | **无函数/宏扩展机制** | ❌ 不能像 QLExpress 那样注册自定义函数。 | 规则复用性差，脚本冗长。 |
